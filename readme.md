...
![formula](https://render.githubusercontent.com/render/math?math=N_{scale}(t)%20=%20N_0%20\left%20(t%20\over%20modifier_{scale}%20\right))

Для большинства инструментов характерен равномерно-темперированный строй, то есть коэффициенты $modifier_{scale}$ обычно равны $2^{-scale / 12}$ где $i \in \mathbb{Z}$. Например, если рассматривать ноту "до" первой октавы как базовую ($N_0$) то характеристика ноты "до" следующей октавы может представлена как $N_{12} (t)= N_0 \left ( t \over 2^{12 / 12} \right)$. Также достоин внимания следующий факт: в качестве базовой ноты может быть выбрана любая нота из диапазона инструмента.
...
$modifier_{scale} = 2^{-scale / 12}$, где *scale* - положение относительно базовой ноты.

*Длина дочернего вейвлета = длина материнского вейвлета* * $modifier_{scale}$
...
Для каждой ноты из списка, генерируем дочерний вейвлет $wavelet_{scale}$, где *scale* - положение относительно базовой ноты. 

Производим [взаимную корреляцию](https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.correlate.html) исходного сигнала (музыкального файла) и дочернего вейвлета, затем домножаем на $|modifier_{scale}|^{-1/2}$. Получаем результат вейвлет-преобразования, записываем в строку в двумерный массив. Повторяем для каждой ноты из списка.
